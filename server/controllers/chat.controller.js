...
const addMembers = asyncHandler(async (req, res, next) => {
    ...
    // only creator of group can add the members
    ...
    // only added the new members ID on members array
    ...
    // In one group 100 members can added
...
const removeMember = asyncHandler(async (req, res, next) => {
    ...
    // only creator of group can remove the members
    if (chat.creator.toString() !== req.user.toString())
        return next(
            new ErrorHandler("You are not allowed to add members", 403)
        );
...
const leaveGroup = asyncHandler(async (req, res, next) => {
    ...
    return res.status(200).json({
        success: true,
        message: "Someone leave the group successfully",
    });
});
